"""
Crimson Policy Generator 2.0 - With IT Glue Integration
Enhanced Flask application with automatic client data population from IT Glue
"""
...
# Import IT Glue integration module
from itglue_integration import ITGlueIntegration

app = Flask(__name__)
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Initialize IT Glue integration
...
# NEW ENDPOINTS FOR IT GLUE INTEGRATION

@app.route('/api/itglue/organizations', methods=['GET'])
def get_organizations():
    """
    Get list of active client organizations from IT Glue
    Used to populate the searchable dropdown
    """
    try:
        organizations = itglue.get_active_organizations()
        return jsonify({
            'success': True,
...
@app.route('/api/itglue/organization/<int:org_id>/profile', methods=['GET'])
def get_organization_profile(org_id):
    """
    Get organization profile and auto-populate form fields
    Returns mapped data ready for form population
    """
    try:
        # Get profile from IT Glue
        profile = itglue.get_organization_profile(org_id)
        ...
        form_data = itglue.map_profile_to_form_data(profile)

        return jsonify({
            'success': True,
            'form_data': form_data,
            'profile': profile  # Include full profile for debugging/display
        })
    except Exception as e:
...
# EXISTING ENDPOINT - Enhanced with IT Glue save option
...
def generate_policy():
    try:
        ...
        form_data = {
            ...
            'itglue_org_id': request.form.get('itglue_org_id', '').strip(),  # NEW: Store org ID for save
            'save_to_itglue': request.form.get('save_to_itglue', 'false') == 'true'  # NEW: Save option
        }
        ...
        # NEW: Save to IT Glue if requested
        if form_data['save_to_itglue'] and form_data['itglue_org_id']:
            try:
                org_id = int(form_data['itglue_org_id'])
                policy_name = f"{form_data['client_name']} - {form_data['policy_type']}"

                compliance_frameworks = []
                if form_data['compliance_requirements']:
                    compliance_frameworks.append(form_data['compliance_requirements'])

                save_result = itglue.save_policy_to_itglue(
                    org_id=org_id,
                ...
                if not save_result.get('success'):
                    logger.error(f"Failed to save to IT Glue: {save_result.get('error')}")
                    # Don't fail the whole request, just log the error
                else:
                    logger.info(f"Policy saved to IT Glue: {save_result.get('document_id')}")
            except Exception as e:
                logger.error(f"Error saving to IT Glue: {str(e)}")
...
def health():
    try:
        ...
        api_key = os.getenv('AZURE_OPENAI_API_KEY')
        endpoint = os.getenv('AZURE_OPENAI_ENDPOINT')
        itglue_backend = os.getenv('ITGLUE_BACKEND_URL')

        health_info = {
            "status": "healthy",
            "version": "2.0.0",
            "openai_configured": bool(api_key and endpoint),
            "itglue_configured": bool(itglue_backend),
            "env_check": {
                "api_key_present": bool(api_key),
                "endpoint_present": bool(endpoint),
                "itglue_backend_present": bool(itglue_backend)
            }
        }
